        ...
        <nav class="flex items-center justify-between flex-wrap bg-red-300 p-6">
            ...
            <div class="w-full block flex-grow lg:flex lg:items-center lg:w-auto">
                <div class="text-sm lg:flex-grow">
                    ...
                    @guest
                    @if (Route::has('login'))
                    <a class="block mt-4 lg:inline-block lg:mt-0 text-white hover:text-blue-500 hover:underline mr-4" href="{{ route('login') }}">{{ __('Iniciar Sesión') }}</a>
                    @endif

                    @if (Route::has('register'))
                    <a class="block mt-4 lg:inline-block lg:mt-0 text-white hover:text-blue-500 hover:underline mr-4" href="{{ route('register') }}">{{ __('Registrarse') }}</a>
                    @endif
                    @else
